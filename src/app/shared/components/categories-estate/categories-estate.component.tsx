...
const CategoriesEstateComponent: FC<Readonly<ICategoriesEstateProps>> = ({
  categoryEstate,
}) => {
  const estateArray = Array.isArray(categoryEstate)
    ? categoryEstate
    : [categoryEstate];
  ...
  return (
    <>
      {estateArray.map((estate, index) => {
        ...
        const background =
          backgroundKey in imagesMap ? imagesMap[backgroundKey] : null;
        ...
        return (
          <Link href="#" key={index}>
            ...
            >
              ...
              />
              <div
                className={`${styles.hoverContent} ${isHovered ? styles.show : ''}`}
              >
