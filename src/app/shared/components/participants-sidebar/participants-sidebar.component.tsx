...
const ParticipantsSidebarComponent: FC<Readonly<IParticipantsSidebarProps>> = ({
  ...
  const getProposals = (participant: IOwner | IBroker | IBuilder) => {
    switch (participant.category) {
      ...
      case 'broker':
        return (
          <p className={styles.proposals}>{participant.proposals} пропозицій</p>
        );
      case 'builder':
      ...
      <div className="flex flex-col md:flex-row lg:flex-col">
        <div className={`${styles.listPeople} flex flex-col`}>
          <div className="flex gap-3">
            ...
            <button
              className={`${classNameButton}  flex items-center gap-1.5 justify-center`}
            >
              20
              <svg
                width="15"
                ...
                return (
                  <li key={index}>
                    <Link href="#" className="flex items-center gap-3">
                      <Image
                        src={image}
                        alt={participant.nameParticipant}
                        width={42}
                        height={42}
                      />
