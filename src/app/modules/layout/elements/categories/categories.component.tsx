...
const CategoriesComponent: FC<Readonly<ICategoriesProps>> = ({
  ...
  return (
    <section
      className={`${styles.categories} flex lg:flex-row flex-col gap-5 p-2.5`}
    >
      <div
        className={`${styles.itemsTable} grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5`}
      >
        {Array.isArray(categoryEstate) ? (
          categoryEstate.length > 0 ? (
            categoryEstate.map((item, index) => (
              <CategoriesEstateComponent
                key={index}
                settings={settings}
                categoryEstate={item}
              />
      ...
      <div className={`${styles.lists} flex gap-3 flex-col`}>
        <CategoriesSidebarComponent
          titleHeader={'Оренда'}
          categories={categoriesOffersForRent}
        />
        <CategoriesSidebarComponent
          titleHeader={'Продаж'}
          categories={categoriesOffersForSale}
        />
      </div>
    </section>
  );
