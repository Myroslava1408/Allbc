...
const ServicesComponent: FC<Readonly<IServicesProps>> = ({
  additionalServices,
}) => {
  const type1Services = additionalServices.filter(
    (item) => item.service.type === 1,
  );
  const type2Services = additionalServices.filter(
    (item) => item.service.type === 2,
  );
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 lg:p-0 p-2 gap-7 justify-center">
          {Array.isArray(type1Services) && type1Services.length > 0 ? (
            type1Services.map((block, blockItem) => {
              const className =
                blockItem === type1Services.length - 1
